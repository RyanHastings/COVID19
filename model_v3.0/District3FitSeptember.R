...
plt_outdir<-"../../../COVID19Response/model_v3.0_out/out/20200908/" # directory for output
...
for (n in 1:10) {
 ...
 # dum4<-rep(0,Nlines)
  ...
  Ccum<-rep(0,Nlines)
  Dday<-rep(0,Nlines)
  Dates<-rep(dates_exp,Nlines/maxt)
  ...
  while (kk<=K) {
    ...
    #  dim4[nn]<-l
      ...
      SepLiftReduction[nn]<-StateSpace[i,j,k,8,t]#lift_rdxn_vec[i]
    #  SepLiftReduction[nn]<-StateSpace[i,j,k,8,t]#lift_rdxn_vec[j]
      Rhosp[nn]<-StateSpace[i,j,k,9,t]
      Rcrit[nn]<-StateSpace[i,j,k,10,t]
  ...
  D.frame<-data.frame(rank=as.factor(rank),day,SepLiftReduction=SepLiftReduction,
