...
K=50
dt=1.0
outdir<-"../../../COVID19Response/model_v3.0_out/out/20200730/"
...
while (kk<=K) {
  ...
  for (t in 1:maxt) {
    ...
    lift_rdxn1[n]<-StateSpace[i,j,k,l,5,t]
    stage5_weeks[n]<-StateSpace[i,j,k,l,6,t]
    Rhosp[n]<-Rhosp_vec[k]
    Rcrit[n]<-Rcrit_vec[l]
...
#   }
# }
D.frame<-data.frame(dim1,dim2,dim3,dim4,day,LiftReduction1=lift_rdxn1,Stage5WeeksUntilZero=stage5_weeks,
...
print( ggplot(D.frame,aes(x=Stage5WeeksUntilZero))+geom_histogram(binwidth=1)+xlim(-1,15) )
ggsave(paste(outdir,"StatewideLiftWeeksUntilZeroHist.png",sep=""))
