...
PhaseFourReduction_vec<-seq(0.35,0.7,0.05)
...
outfile<-'StateSpace_stage4.RData' # output filename
...
# set up state space
StateSpace<-array(0.0,dim=c(length(PhaseFourReduction_vec),length(stage5_weeks_vec),length(Rhosp_vec),length(Rcrit_vec),nvars,maxt))
#haseFourReduction_vec<-PhaseThreeReduction_vec

#for (R0i in 1:length(R0_vec)) {
for (i in 1:length(PhaseFourReduction_vec)) {
  ...
  for (j in 1:length(stage5_weeks_vec)) {
    ...
    for (Rhosp_vec_i in 1:length(Rhosp_vec)) {
      for (Rcrit_vec_i in 1:length(Rcrit_vec)) {            
        ...
        PhaseFourReduction<-PhaseFourReduction_vec[i]
        stage5_weeks<-stage5_weeks_vec[j]
        RhospPhaseFour<-Rhosp_vec[Rhosp_vec_i]
        RcritPhaseFour<-Rcrit_vec[Rcrit_vec_i]
        ...
        print(paste("lift1=",PhaseFourReduction,",weeks=",stage5_weeks,"Rcrit=",RcritPhaseFour,"Rhosp=",RhospPhaseFour))
        source("DetermineStateSpaceStageThree_initialization.R")
        source("DetermineStateSpaceStageFour_dynamics.R")
        source("DetermineStateSpaceStageFour_out.R")
