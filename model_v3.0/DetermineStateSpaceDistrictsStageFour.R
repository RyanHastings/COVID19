...
lift_rdxn_vec<-c(seq(0.35,0.7,0.05)) # field of phase two NPI reduction rate over which to simulate
...
outfile<-'StateSpace_StageFour' # beginning of state space filename
    ...
    for (weeks_j in 1:length(stage5_weeks_vec)) {
      ...
      for (Rhosp_vec_i in 1:length(Rhosp_vec)) {
        for (Rcrit_vec_i in 1:length(Rcrit_vec)) {
          ...
          StageFourReduction<-lift_rdxn_vec[lift_rdxn_i]
          RhospStageFour<-Rhosp_vec[Rhosp_vec_i]
          RcritStageFour<-Rcrit_vec[Rcrit_vec_i]
          ...
          print(paste("StageFourReduction=",StageFourReduction,"weeks=",stage5_weeks,"Rcrit=",
                      RcritStageFour,"Rhosp=",RhospStageFour,',district',n))#"intervention_time=",intervention_time,))#
          
          # run the model
          source("DetermineDistrictStateSpace_initialization.R")
          source("DetermineStateSpaceDistrictsStageFour_dynamics.R")
          source("DetermineStateSpaceDistrictsStageFour_out.R")
        }
      }
    }
  }
