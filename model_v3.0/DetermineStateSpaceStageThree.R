...
outfile<-'StateSpace_stage3.RData' # output filename
...
PhaseFourReduction_vec<-PhaseThreeReduction_vec

#for (R0i in 1:length(R0_vec)) {
for (i in 1:length(PhaseThreeReduction_vec)) {
  
  #PhaseFourReduction_vec<-seq(0.5*PhaseThreeReduction_vec[i],PhaseThreeReduction_vec[i],0.05)
  #PhaseFourReduction_vec<-
  
  for (j in 1:length(PhaseFourReduction_vec)) {
    ...
    for (Rhosp_vec_i in 1:length(Rhosp_vec)) {
      for (Rcrit_vec_i in 1:length(Rcrit_vec)) {            
        ...
        PhaseThreeReductionA<-PhaseThreeReduction_vec[i]
        PhaseThreeReductionB<-PhaseThreeReductionA
        PhaseFourReduction<-PhaseThreeReduction_vec[j]

        RhospPhaseThree<-Rhosp_vec[Rhosp_vec_i]
        RcritPhaseThree<-Rcrit_vec[Rcrit_vec_i]
        ...
        print(paste("lift1=",PhaseThreeReductionA,",lift2=",PhaseFourReduction,"Rcrit=",RcritPhaseThree,"Rhosp=",RhospPhaseThree))
        source("DetermineStateSpaceStageThree_initialization.R")
        source("DetermineStateSpaceStageThree_dynamics.R")
        source("DetermineStateSpaceStageThree_out.R")
