...
###########################################################
# Ryan Hastings, 23 May 2020..29 june for stage five
###########################################################
...
for (t in 2:maxt) {
  ...
  if (t<intervention_time) {
    ...
    Rhosp<-RhospStageFour
    Rcrit<-RcritStageFour
  } else if (t>=StageFiveDay & t<AugustDay) {
    Rt<-(1-JulyReduction)*R0
    Rhosp<-RhospStageFive
    Rcrit<-RcritStageFive
  } else if (t>=AugustDay & t<AugustDay+stage5_weeks*7) {
    Rt<-R0*(1-JulyReduction*(stage5_weeks*7-(t-AugustDay))/(stage5_weeks*7))
    Rhosp<-RhospStageFive
    Rcrit<-RcritStageFive
    # print(t-PhaseFiveDay)
    # print((stage5_weeks*7-(t-PhaseFiveDay))/(stage5_weeks*7))
    # print(Rt)
  } else if (t>=AugustDay+stage5_weeks*7) {
    Rt<-R0
    Rhosp<-RhospStageFive
    Rcrit<-RcritStageFive
  }
