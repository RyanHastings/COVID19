...
##########################################################
# Ryan Hastings, 4 May 2020 / 15 Jun
##########################################################
...
lift_rdxn_vec<-c(seq(0.35,0.7,0.05)) # field of NPI reduction rate over which to simulate
Rhosp_vec<-seq(0.01,0.1,0.005)#0.06 # hospitalization rate
Rcrit_vec<-seq(0.01,0.03,0.001)#0.024 # critical rate
plt_outdir<-"../../../COVID19Response/model_v3.0_out/out/20200828/" # directory for output
...
for (n in 10) {
  #load(paste('StateSpace_StageFive_District',n,'.RData',sep=""))
  load(paste('StateSpace_August_District',n,'.Rdata',sep=""))
  ...
  AugLiftReduction<-rep(0,Nlines)
  SepLiftReduction<-rep(0,Nlines)
  Rhosp<-rep(0,Nlines)
  Rcrit<-rep(0,Nlines)
  Dflat<-rep(0,Nlines)
  dim1<-rep(0,Nlines)
  ...
  while (kk<=K) {
    ...
    for (t in 1:maxt) {
      ...
      AugLiftReduction[nn]<-StateSpace[i,j,k,l,7,t]#lift_rdxn_vec[i]
      SepLiftReduction[nn]<-StateSpace[i,j,k,l,8,t]#lift_rdxn_vec[j]
      Rhosp[nn]<-StateSpace[i,j,k,l,9,t]
      Rcrit[nn]<-StateSpace[i,j,k,l,10,t]
  ...
  D.frame<-data.frame(rank=as.factor(rank),day,AugLiftReduction=AugLiftReduction,SepLiftReduction=SepLiftReduction,
  ...
  print( ggplot(D.frame,aes(x=AugLiftReduction))+geom_histogram(binwidth=0.05)+xlim(0.0,0.8) )+ggtitle(paste('district',n))
  ggsave(paste(plt_outdir,"district",n,"_AugLiftReductionHist.png",sep=""))
  print( ggplot(D.frame,aes(x=SepLiftReduction))+geom_histogram(binwidth=1)+xlim(0.0,0.8) )+ggtitle(paste('district',n))
  ggsave(paste(plt_outdir,"district",n,"_SepLiftReductionHist.png",sep=""))
  ...
  ggsave(paste(plt_outdir,"district",n,"_RhospHist.png",sep=""))
  ...
  ggsave(paste(plt_outdir,"district",n,"_RcritHist.png",sep=""))
  ...
  ggsave(paste(plt_outdir,"district",n,"_DeathsPerDay.png",sep=""))
  ...
  ggsave(paste(plt_outdir,"district",n,"_Hospitalized.png",sep=""))
  ...
  ggsave(paste(plt_outdir,"district",n,"_Critical.png",sep=""))
