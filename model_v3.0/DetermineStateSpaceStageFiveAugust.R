...
StateSpace<-array(0.0,dim=c(length(Rcrit_vec),length(Rhosp_vec),length(stage5_weeks_vec),length(AugReduction_vec),nvars,maxt))
  ...
  for (Rcrit_vec_i in 1:length(Rcrit_vec)) {
    for (j in 1:length(stage5_weeks_vec)) {
      for (i in 1:length(AugReduction_vec)) {
        ...
        print(paste("Rcrit=",RcritAugust,"Rhosp=",RhospAugust,"weeks=",stage5_weeks,"lift=",AugReduction))
        source("DetermineStateSpaceStageThree_initialization.R")
        source("DetermineStateSpaceStageFiveAugust_dynamics.R")
        source("DetermineStateSpaceStageFiveAugust_out.R")
