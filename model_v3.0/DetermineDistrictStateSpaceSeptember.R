...
######################################################################
# Ryan Hastings, 23 May 2020 / 12 June 2020 / 28 aug 2020
######################################################################
...
AugReduction_vec<-c(0.55,0.5,0.65,0.25,0.7,0.65,0.35,0.55,0.65,0.5)
...
RhospAug<-c(0.1,0.04,0.1,0.04,0.08,0.05,0.05,0.05,0.06,0.04)
RcritAug<-c(0.027,0.01,0.033,0.01,0.029,0.011,0.01,0.013,0.015,0.017)
...
for (n in 1:10) {
  ...
  AugReduction<-AugReduction_vec[n]
  ...
  for (lift_rdxn_i in 1:length(lift_rdxn_vec)) {
    #for (weeks_j in 1:length(lift_rdxn_vec)) {
      ...
      for (Rhosp_vec_i in 1:length(Rhosp_vec)) {
        for (Rcrit_vec_i in 1:length(Rcrit_vec)) {
          ...
          SepReduction<-lift_rdxn_vec[lift_rdxn_i]
          #SepReduction<-lift_rdxn_vec[weeks_j]
          ...
          print(paste("StageFourReduction=",AugReduction,"weeks=",SepReduction,"Rcrit=",
                      RcritSep,"Rhosp=",RhospSep,',district',n))#"intervention_time=",intervention_time,))#
          
          # run the model
          source("DetermineDistrictStateSpace_initialization.R")
          source("DetermineDestrictStateSpaceSeptember_dynamics.R")
          if (is.na(Rhosp)) { stop() }
          source("DetermineDistrictStateSpaceSeptember_out.R")
       # }
      }
    }
  }
