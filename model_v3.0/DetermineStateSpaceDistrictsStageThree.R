...
#lift_rdxn_vec<-c(seq(0.0,0.7,0.05)) # field of phase two NPI reduction rate over which to simulate
nvars<-8 # number of variables for output (you probably don't want to change this unless you want a huge coding headache)
state_name="Indiana"
...
StageFourDate<-as.Date("2020-06-14")
...
outfile<-'StateSpace_StageThree' # beginning of state space filename

StageThreeReductionA_vec<-seq(0.0,0.7,0.05)
...
#   StageTwoReductionB<-StageTwoReductionA
...
#   
#   # set up state space
#   StateSpace<-array(0.0,dim=c(length(StageThreeReductionA_vec),length(Rhosp_vec),length(Rcrit_vec),nvars,maxt))
#   # count=0
#   for (lift_rdxn_i in 1:length(StageThreeReductionA_vec)) {
...
#         StageThreeReductionA<-StageThreeReductionA_vec[lift_rdxn_i]
#         StageThreeReductionB<-StageThreeReductionA
#         RhospStageThree<-Rhosp_vec[Rhosp_vec_i]
#         RcritStageThree<-Rcrit_vec[Rcrit_vec_i]
#         
#         StageFourReduction<-0.5*StageThreeReductionA
#         
#         # to keep track of where we are in the simulation
#         print(paste("StageThreeReductionA=",StageThreeReductionA,"Rcrit=",
#                     RcritStageThree,"Rhosp=",RhospStageThree,',district',n))#"intervention_time=",intervention_time,))#
#         
#         # run the model
#         source("DetermineDistrictStateSpace_initialization.R")
#         source("DetermineStateSpaceDistrictsStageThree_dynamics.R")
...
for (n in 4:10) {
  ...
  # set up state space
  StateSpace<-array(0.0,dim=c(length(StageThreeReductionA_vec),
                              length(StageThreeReductionA_vec),
  ...
  for (lift_rdxn_i in 1:length(StageThreeReductionA_vec)) {
    ...
    StageThreeReductionA<-StageThreeReductionA_vec[lift_rdxn_i]
    StageThreeReductionB<-StageThreeReductionA
    # StageFourReduction_vec<-seq(StageThreeReductionA*0.5,StageTwoReductionB)
    StageFourReduction_vec<-seq(StageThreeReductionA*0.5,0.7,0.05)
    
    for (lift_rdxn_j in 1:length(StageFourReduction_vec)) {

      for (Rhosp_vec_i in 1:length(Rhosp_vec)) {
        for (Rcrit_vec_i in 1:length(Rcrit_vec)) {
          # iterate through lift reduction, Rhosp, and Rcrit vectors
          # StageThreeReductionB<-StageThreeReductionB_vec[lift_rdxn_j]
          StageFourReduction<-StageFourReduction_vec[lift_rdxn_j]
          RhospStageThree<-Rhosp_vec[Rhosp_vec_i]
          RcritStageThree<-Rcrit_vec[Rcrit_vec_i]
          ...
          # to keep track of where we are in the simulation
          print(paste("StageThreeReductionA=",StageThreeReductionA,"StageThreeReductionB=",StageFourReduction,"Rcrit=",
                      RcritStageThree,"Rhosp=",RhospStageThree,',district',n))#"intervention_time=",intervention_time,))#

          # run the model
          source("DetermineDistrictStateSpace_initialization.R")
          source("DetermineStateSpaceDistrictsStageThree_dynamics.R")
          source("DetermineStateSpaceDistrictsStateThree_out.R")
        }
      }
    }
  }
