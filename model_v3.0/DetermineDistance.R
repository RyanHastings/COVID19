...
###########################################################
# Depracated by writing of Fit3curves.R -- RMH 5 may 2020
###########################################################
...
MatchVar<-13 # which variable to match on (i.e. which one is the deceased curve)
...
track_filename<-"DeathCurves/COVID_Deaths_Per_Day_0416.csv"
...
CumDeathCount<-c(dum$DeathCount[1])
for (i in 2:nrow(dum)) {
  CumDeathCount<-append(CumDeathCount,CumDeathCount[i-1]+dum$DeathCount[i])
}
dum<-dum%>%bind_cols(CumDeathCount=CumDeathCount)
...
i1<-as.numeric(as.Date(dum$Date_of_death[1],format='%m/%d/%y')-DayZero+2)
i2<-as.numeric(as.Date(dum$Date_of_death[nrow(dum)-1],format="%m/%d/%y")-DayZero+1)

#dates<-seq.Date(DayZero,as.Date(paste(dum$Date[nrow(dum)],"2020",sep="-"),format="%d-%b-%Y"),"days")
dates<-seq.Date(DayZero,as.Date(dum$Date_of_death[nrow(dum)-1],format="%m/%d/%y"),"days")
dates_exp<-seq.Date(DayZero,DayZero+maxt-1,"days")


track<-c(rep(0,length(dates)))
track[i1:i2]<-dum$DeathCount[1:(nrow(dum)-1)]#dum$CumDeathCount
ntimes<-length(track)

# get distances
StateSpace.ndims<-dim(StateSpace)
D<-array(0.0,dim=c(StateSpace.ndims[1],StateSpace.ndims[2]))
for (t in i1:i2) {#1:ntimes) {
...
for (i in 1:length(R0_vec)) {
  for (j in 1:length(intervention_R_rdxn_vec)) {
    for (t in 1:maxt) {
      ...
      Ccum[n]<-StateSpace[i,j,11,t]
      percent_infected<-StateSpace[i,j,12,t]
      Dday[n]<-StateSpace[i,j,13,t]
    #  Dates[n]<-dates_exp[t]
      n<-n+1
    }
  }
...
print(R0_vec[dum23[1]])
print(intervention_R_rdxn_vec[dum23[2]])

if (plot==1) {
print(ggplot()+geom_point(data=df.track,aes(x=Dates,y=Deceased))+
        geom_line(data=D.frame%>%filter(dim1==dumi1,dim2==dumj1),aes(x=Dates,y=DeathPerDay))+
        xlim(DayZero,DayZero+100)+ylim(0,150)+ggtitle(paste("R0=",R0_vec[dumi1])) )
print(ggplot()+geom_point(data=df.track,aes(x=Dates,y=Deceased))+
        geom_line(data=D.frame%>%filter(dim1==dumi1,dim2==dumj1+2),aes(x=Dates,y=DeathPerDay))+
        xlim(DayZero,DayZero+100)+ylim(0,150)+ggtitle(paste("R0=",R0_vec[dumi1])) )
# print(ggplot()+geom_point(data=df.track,aes(x=Dates,y=Deceased))+
#         geom_line(data=D.frame%>%filter(dim1==dumi1+1,dim2==1),aes(x=Dates,y=DeathPerDay,color='no NPI'))+
#         geom_line(data=D.frame%>%filter(dim1==dumi1+1,dim2==11),aes(x=Dates,y=DeathPerDay,color='50% reduction'))+
#         geom_line(data=D.frame%>%filter(dim1==dumi1+1,dim2==17),aes(x=Dates,y=DeathPerDay,color='80% reduction'))+
#         geom_line(data=D.frame%>%filter(dim1==dumi1+1,dim2==19),aes(x=Dates,y=DeathPerDay,color='90% reduction')) )
