...
StateSpace_filename<-"StateSpace_August2.Rdata"
...
K=50
dt=1.0
outdir<-"../../../COVID19Response/model_v3.0_out/out/20200828/"
...
while (kk<=K) {
  ...
  for (t in 1:maxt) {
    ...
    SepReduction[n]<-StateSpace[i,j,k,l,6,t]
    Rhosp[n]<-Rhosp_vec[i]
    Rcrit[n]<-Rcrit_vec[j]
...
#   }
# }
D.frame<-data.frame(dim1,dim2,dim3,dim4,day,AugLiftReduction=AugReduction,SepLiftReduction=SepReduction,
...
print( ggplot(D.frame,aes(x=SepLiftReduction))+geom_histogram(binwidth=1)+xlim(-1,15) )
