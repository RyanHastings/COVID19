...
JulyReduction_vec<-seq(0.7,0.0,-0.05)#seq(0,0.7,0.05)
...
StateSpace<-array(0.0,dim=c(length(Rcrit_vec),length(Rhosp_vec),length(stage5_weeks_vec),length(JulyReduction_vec),nvars,maxt))
  ...
  for (Rcrit_vec_i in 1:length(Rcrit_vec)) {
    for (j in 1:length(stage5_weeks_vec)) {
      for (i in 1:length(JulyReduction_vec)) {
        ...
        PhaseFiveReduction<-JulyReduction_vec[i]
        stage5_weeks<-stage5_weeks_vec[j]
        RhospPhaseFive<-Rhosp_vec[Rhosp_vec_i]
        RcritPhaseFive<-Rcrit_vec[Rcrit_vec_i]
        ...
        print(paste("Rcrit=",RcritPhaseFive,"Rhosp=",RhospPhaseFive,"weeks=",stage5_weeks,"lift=",PhaseFiveReduction))
        source("DetermineStateSpaceStageThree_initialization.R")
        source("DetermineStateSpaceStageFiveJuly_dynamics.R")
        source("DetermineStateSpaceStageFiveJuly_out.R")
