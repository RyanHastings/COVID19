...
JulyReduction<-0.55
AugReduction_vec<-seq(0.7,0.0,-0.05)#seq(0,0.7,0.05)
...
SeptemberDate<-as.Date("2020-08-25")
...
outfile<-'StateSpace_August2.RData' # output filename
...
StateSpace<-array(0.0,dim=c(length(Rcrit_vec),length(Rhosp_vec),length(SepReduction_vec),length(AugReduction_vec),nvars,maxt))
  ...
  for (Rcrit_vec_i in 1:length(Rcrit_vec)) {
    for (j in 1:length(SepReduction_vec)) {
      for (i in 1:length(AugReduction_vec)) {
        ...
        print(paste("Rcrit=",RcritAugust,"Rhosp=",RhospAugust,"weeks=",SepReduction,"lift=",AugReduction))
        source("DetermineStateSpaceStageThree_initialization.R")
        source("DetermineStateSpaceStageFiveAugust2_dynamics.R")
        source("DetermineStateSpaceStageFiveAugust2_out.R")
