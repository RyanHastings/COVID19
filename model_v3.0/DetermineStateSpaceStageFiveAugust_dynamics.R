...
for (t in 2:maxt) {
  ...
  } else if (t>=PhaseFiveDay & t<AugustDay) {
    Rt<-(1-PhaseFiveReduction)*R0
    Rhosp<-RhospPhaseFive
    Rcrit<-RcritPhaseFive
  } else if (t>=AugustDay & t<SeptemberDay) {
    Rt<-(1-AugReduction)*R0
    Rhosp<-RhospAugust
    Rcrit<-RcritAugust
  } else if (t>=SeptemberDay & t<SeptemberDay+stage5_weeks*7) {
    Rt<-R0*(1-AugReduction*(stage5_weeks*7-(t-SeptemberDay))/(stage5_weeks*7))
    Rhosp<-RhospPhaseFive
    Rcrit<-RcritPhaseFive
  } else if (t>=SeptemberDay+stage5_weeks*7) {
    Rt<-R0
  }
