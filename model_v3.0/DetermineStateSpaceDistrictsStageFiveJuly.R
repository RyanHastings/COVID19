...
Rhosp_vec<-seq(0.01,0.1,0.01)#0.06 # hospitalization rate
...
outfile<-'StateSpace_StageFive' # beginning of state space filename
    ...
    for (weeks_j in 1:length(stage5_weeks_vec)) {
      ...
      for (Rhosp_vec_i in 1:length(Rhosp_vec)) {
        for (Rcrit_vec_i in 1:length(Rcrit_vec)) {
          ...
          JulyReduction<-lift_rdxn_vec[lift_rdxn_i]
          RhospStageFive<-Rhosp_vec[Rhosp_vec_i]
          RcritStageFive<-Rcrit_vec[Rcrit_vec_i]
          ...
          print(paste("StageFourReduction=",JulyReduction,"weeks=",stage5_weeks,"Rcrit=",
                      RcritStageFive,"Rhosp=",RhospStageFive,',district',n))#"intervention_time=",intervention_time,))#
          
          # run the model
          source("DetermineDistrictStateSpace_initialization.R")
          source("DetermineStateSpaceDistrictsStageFiveJuly_dynamics.R")
          source("DetermineStateSpaceDistrictsStageFiveJuly_out.R")
        }
      }
    }
  }
